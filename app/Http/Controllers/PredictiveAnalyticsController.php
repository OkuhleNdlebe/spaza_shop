...
class PredictiveAnalyticsController extends Controller
{
    ...
    private function generateSalesPredictions()
    {
        ...
        foreach ($products as $product) {
            ...
            $stores = Store::all();
            if ($stores->isEmpty()) {
                $stores = collect([null]); // Fallback if no stores
            }
    ...
    public function createSamplePredictions()
    {
        try {
            ...
            if (!$store || !$product) {
                return redirect()->route('analytics.dashboard')
                    ->with('error', 'Need at least one store and one product');
            }
            ...
            return redirect()->route('analytics.dashboard')
                ->with('success', "Created {$predictionCount} predictions and {$forecastCount} forecasts");
